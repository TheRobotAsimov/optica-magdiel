...
import { Edit, Trash2, Eye, ShoppingCart, CreditCard, Glasses, User, X, Phone, MapPin, Globe, AlertCircle, Users } from 'lucide-react';
...
const ClientList = () => {
      ...
      try {
        ...
        allPagos.forEach(p => {
          if (p.estatus === 'Pendiente' || p.estatus === 'Atrasado') {
            ...
            if (venta) pendingClients.add(venta.idcliente);
          }
        });
  ...
  const handleRequestAction = (action, clientId) => {
    Swal.fire({
      title: `Solicitar ${action === 'delete' ? 'Eliminación' : 'Edición'}`,
      input: 'textarea',
      inputLabel: 'Motivo de la solicitud',
      inputPlaceholder: `Describe por qué deseas ${action === 'delete' ? 'eliminar' : 'editar'} este cliente...`,
      ...
      if (result.isConfirmed) {
        try {
          const mensaje = `Solicitud de ${action === 'delete' ? 'eliminación' : 'edición'} - Cliente ID: ${clientId}, Motivo: ${result.value} - Solicitado por: ${user.nombre} ${user.paterno}`;
  ...
  const handleDelete = (clientId) => {
    if (user.rol === 'Asesor' || user.rol === 'Optometrista') {
      handleRequestAction('delete', clientId);
      return;
    }
    baseHandleDelete(clientId, {
      successText: 'El cliente ha sido eliminado.',
      errorText: 'Hubo un error al eliminar el cliente.'
    });
  };

  const handleEdit = (clientId) => {
    if (user.rol === 'Asesor' || user.rol === 'Optometrista') {
      handleRequestAction('edit', clientId);
      return;
    }
  ...
  const handleView = async (clientId) => {
    try {
      ...
      setPagos(allPagos.filter(p => clientVentas.some(v => v.folio === p.folio)));
      setLentes(allLentes.filter(l => clientVentas.some(v => v.folio === l.folio)));
      setPacientes(allPacientes.filter(p => p.idcliente === clientId));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <ListHeader
          title="CLIENTES"
          subtitle="Listado de clientes registrados en el sistema"
        />
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <ListAvatar
                        initials={`${client.nombre.charAt(0)}${client.paterno.charAt(0)}`}
                        isWarning={clientsWithPending.has(client.idcliente)}
                      />
                      <div className="ml-4">
            ...
            <div className="bg-white rounded-2xl max-w-5xl w-full shadow-2xl max-h-[90vh] overflow-hidden flex flex-col animate-in">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h2 className="text-3xl font-bold text-white">Detalles del Cliente</h2>
              ...
              <div className="overflow-y-auto p-8 space-y-8">
                <div className="relative">
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {[
                        { label: 'Nombre Completo', value: `${clientDetails.nombre} ${clientDetails.paterno} ${clientDetails.materno}`, icon: User },
                        { label: 'Edad', value: `${clientDetails.edad} años`, icon: User },
                        { label: 'Sexo', value: clientDetails.sexo === 'M' ? 'Masculino' : 'Femenino', icon: User },
                        { label: 'Teléfono 1', value: clientDetails.telefono1, icon: Phone },
                        { label: 'Domicilio 1', value: clientDetails.domicilio1, icon: MapPin },
                        { label: 'Teléfono 2', value: clientDetails.telefono2, icon: Phone },
                        { label: 'Domicilio 2', value: clientDetails.domicilio2, icon: MapPin },
                      ].map((item, idx) => (
                        <div key={idx} className="flex items-start space-x-3">
                          <div className="bg-blue-100 p-2 rounded-lg mt-0.5"><item.icon className="h-4 w-4 text-blue-600" /></div>
                          <div>
                      ...
                      <div className="flex items-start space-x-3">
                        <div className="bg-blue-100 p-2 rounded-lg mt-0.5"><Globe className="h-4 w-4 text-blue-600" /></div>
                        <div>
                          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Maps URL</p>
                          <a href={clientDetails.map_url} target="_blank" rel="noopener noreferrer" className="text-base font-semibold text-blue-600 hover:text-blue-700 underline mt-1 inline-block">Ver ubicación</a>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="relative">
                  ...
                  <div className={`rounded-xl p-6 border ${hasPendingItems ? 'bg-gradient-to-br from-red-50 to-orange-50 border-red-100' : 'bg-gray-50 border-gray-100'}`}>
                    <div className="flex items-center space-x-3 mb-6">
                      <div className={`p-2 rounded-lg ${hasPendingItems ? 'bg-red-600' : 'bg-gray-600'}`}><AlertCircle className="h-5 w-5 text-white" /></div>
                      <h3 className="text-xl font-bold text-gray-900">Pagos y Lentes Pendientes o Atrasados</h3>
                    </div>
                    <div className="space-y-3">
                      {pendingVentas.map(v => (
                        <div key={v.folio} className="flex items-center space-x-3 bg-white p-4 rounded-xl border-l-4 border-red-500 shadow-sm">
                          <div className="bg-red-100 p-2 rounded-lg"><ShoppingCart className="h-5 w-5 text-red-600" /></div>
                          <div className="flex-1">
                            <p className="font-semibold text-gray-900">Venta - Folio: {v.folio}</p>
                            <p className="text-sm text-gray-600">Fecha: {formatDate(v.fecha)} • Monto: <span className="font-semibold">${v.total}</span></p>
                          </div>
                          <span className="px-3 py-1 bg-red-100 text-red-700 text-xs font-semibold rounded-full">{v.estatus}</span>
                        </div>
                      ))}
                      {pendingPagos.map(p => (
                        <div key={p.idpago} className="flex items-center space-x-3 bg-white p-4 rounded-xl border-l-4 border-orange-500 shadow-sm">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-600">Fecha: {formatDate(p.fecha)} • Monto: <span className="font-semibold">${p.cantidad}</span></p>
                          </div>
                          <span className="px-3 py-1 bg-orange-100 text-orange-700 text-xs font-semibold rounded-full">{p.estatus}</span>
                        </div>
                      ))}
                      {pendingLentes.map(l => (
                        <div key={l.idlente} className="flex items-center space-x-3 bg-white p-4 rounded-xl border-l-4 border-yellow-500 shadow-sm">
                          <div className="bg-yellow-100 p-2 rounded-lg"><Glasses className="h-5 w-5 text-yellow-600" /></div>
                          <div className="flex-1">
                            <p className="font-semibold text-gray-900">Lente - Folio: {l.folio}</p>
                            <p className="text-sm text-gray-600">Fecha Entrega: {formatDate(l.fecha_entrega)}</p>
                          </div>
                          <span className="px-3 py-1 bg-yellow-100 text-yellow-700 text-xs font-semibold rounded-full">{l.estatus}</span>
                        </div>
                      ))}
                      {!hasPendingItems && (
                ...
                <div className="relative">
                  ...
                  <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-6 border border-indigo-100">
                    <div className="flex items-center space-x-3 mb-6">
                      <div className="bg-indigo-600 p-2 rounded-lg"><Users className="h-5 w-5 text-white" /></div>
                    ...
                    {pacientes.length > 0 ? (
                      <div className="space-y-3">
                        {pacientes.map(p => (
                          <div key={p.idpaciente} className="flex items-center space-x-3 bg-white p-4 rounded-xl shadow-sm">
                            <div className="flex-shrink-0 h-10 w-10 bg-gradient-to-br from-indigo-400 to-purple-500 rounded-full flex items-center justify-center text-white font-semibold text-sm shadow-md">
                              {p.nombre.charAt(0)}{p.paterno.charAt(0)}
                            </div>
                            <div className="flex-1">
                              <p className="font-semibold text-gray-900">{p.nombre} {p.paterno}</p>
                      ...
                      <div className="text-center py-8">
                        <div className="inline-block bg-gray-100 p-3 rounded-full mb-3"><Users className="h-8 w-8 text-gray-400" /></div>
              ...
              <div className="bg-gray-50 px-8 py-4 border-t border-gray-200">
                <div className="flex justify-end">
                  <button onClick={() => setShowModal(false)} className="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl font-semibold transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105">Cerrar</button>
                </div>
              </div>
            </div>
          </div>
